...
def wrapper_opt(P):
    ...
    cmip5_scenarios = pd.read_csv('../Data/cmip5/scenario_names_train.csv').name.to_list()
    ...
    for indicator in indicators:
        data_ind[indicator] = pd.read_csv('../Data/indicator_files/%s_indicators.csv' % indicator)
            #indicators are saved from year 2000 similarlr df_Q and demand data are selected

    num = 0
    for gridi in range(97): #ranges utp 67
        for gridj in range(36): #ranges upto 36
            ...
            res_rule = json.load(open('../Data/res_new.json'))
...
'''the following may be run for serial processing'''
