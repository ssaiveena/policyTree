...
sys.path.append('..')
import matplotlib
matplotlib.use('TkAgg')  # Or 'QtAgg' if TkAgg doesn't work
...
def func(gridi,gridj,w):
    ...
    for years in range(n_y-50):
      ...
      def make_callback(layers, K, demand, Q, T, dowy):
          def callback_func(xk, convergence_val=None):
              cfs_to_tafd = 2.29568411 * 10 ** -5 * 86400 / 1000
              val = train(xk, layers, K, demand, Q['ORO_inflow_cfs'].values * cfs_to_tafd, T, dowy)
              convergence.append(val)

          return callback_func

      callback = make_callback(layers, K, demand, Q, T, dowy)

      res = DE(train, args=(layers, K, demand, Q['ORO_inflow_cfs'].values * cfs_to_tafd, T, dowy),  maxiter=10000,bounds=np.tile(weight_bounds, (num_weights,1)), callback=callback)
      plt.plot(convergence)
      plt.xlabel("Iteration")
      plt.ylabel("Objective Function Value")
      plt.title("DE Convergence for one selected scenario")
      # plt.grid(True)
      plt.tight_layout()
      plt.show()
